...
const addToWishlist = async (req, res) => {
  ...
  const user = await User.findOneAndUpdate({ id: req.actor.userId }, { $addToSet: { wishlist: productId } }, { new: true }).lean();
...
const removeFromWishlist = async (req, res) => {
  ...
  const productId = Number(req.params.productId);
  const user = await User.findOneAndUpdate({ id: req.actor.userId }, { $pull: { wishlist: productId } }, { new: true }).lean();
